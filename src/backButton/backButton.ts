...
export class BackButtonHardware implements IBackButtonHardware {
  ...
  private initQueue(): void {
    this.$queue = new PriorityQueue<BackButtonListener>(
      BackButtonHardware.sortByPriority
    );
  }

  private static sortByPriority(
    a: BackButtonListener,
    b: BackButtonListener
  ): number {
  ...
  unsubscribe(...eventListener: BackButtonListener[]) {
    eventListener.forEach((event) => {
      const index: number = this.$queue
        .toArray()
        .findIndex((e) => e.unique === event.unique);
      if (index !== -1) {
  ...
  private publishEvent(event: BackButtonListenerEvent): void {
    const eventQueue: PriorityQueueType<BackButtonListener> =
      this.$queue.clone();
  ...
  private emitEvent(
    event: BackButtonListenerEvent,
    queue: PriorityQueueType<BackButtonListener>
  ): void {
